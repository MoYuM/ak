...
export default (function () { return __awaiter(void 0, void 0, void 0, function () {
    var now, time, _a, akConfigSuccess, akConfig, akConfigError, _b, packageJsonSuccess, packageJson, packageJsonError, projectPath, output, pages, task;
    return __generator(this, function (_c) {
        switch (_c.label) {
            ...
            case 2:
                ...
                output = execSync("cd ".concat(projectPath, " && git grep --name-only \"").concat(packageJson.name, "\" -- */package.json"), { encoding: "utf-8" });
                pages = output.split("\n").filter(Boolean).map(function (i) { return i.replace("/package.json", ""); });
                if (!(pages && pages.length > 0)) return [3 /*break*/, 4];
                task = pages.map(function (i) {
                    return cp(path.join(cwd(), "miniprogram_dist"), path.join(projectPath, i, "miniprogram_npm", packageJson.name), { recursive: true });
                });
